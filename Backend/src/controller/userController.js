...
import { getNotificationDetails, getTicketDetails, isUserExist, signUp } from "../DAO/user.dao.js";
import userModel from "../models/user.model.js";
import { generateToken } from "../service/cookieToken.js";
import { hashedPassword } from "../service/hashPassword.js";
...
export const otpVerify = async (req, res) => {
  try {
    ...
    if (user.verificationExpiryTime.getTime() <= Date.now()) {
      console.log(user.verificationExpiryTime-Date.now());
    ...
    return badResponse(res, 400, "Invalid OTP");
  } catch (error) {
